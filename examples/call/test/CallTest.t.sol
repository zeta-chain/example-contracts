...
contract CallTest is FoundrySetup {
    ...
    function setUp() public override {
        ...
        connected = new Connected(payable(address(evmSetup.wrapGatewayEVM(chainIdETH))));
    ...
    function test_crosschain_evm_to_zeta() public {
        ...
        RevertOptions memory revertOptions = _getDefaultRevertOptions(address(connected), false, address(universal));
    ...
    function test_crosschain_zeta_to_evm() public {
        bytes memory encodedMessage = abi.encode(message);
        RevertOptions memory revertOptions = _getDefaultRevertOptions(address(universal), true, address(universal));
    ...
    function test_deposit_native_from_evm() public {
        RevertOptions memory revertOptions = _getDefaultRevertOptions(address(connected), true, address(universal));
        ...
        assertEq(IZRC20(eth_eth.zrc20).balanceOf(address(universal)), initialReceiverBalance + AMOUNT);
    }
    ...
    function test_deposit_erc20_from_evm() public {
        ...
        uint256 initialReceiverBalance = IERC20(usdc).balanceOf(address(universal));
        ...
        assertEq(IZRC20(usdc_eth.zrc20).balanceOf(address(universal)), initialReceiverBalance + amount);
    }
    ...
    function test_withdraw_zrc20_from_zeta() public {
        ...
        if (tokenInfo.isGasToken) {
            ...
            initialBalance = IERC20(tokenInfo.asset).balanceOf(address(connected));
        }
        ...
        if (tokenInfo.isGasToken) {
            ...
            assertEq(IERC20(tokenInfo.asset).balanceOf(address(connected)), initialBalance + AMOUNT);
        }
    }
    ...
    function test_withdrawAndCall_zrc20_from_zeta() public {
        ...
        if (tokenInfo.isGasToken) {
            ...
            initialReceiverBalance = IERC20(tokenInfo.asset).balanceOf(address(connected));
        }
        ...
        if (tokenInfo.isGasToken) {
            assertEq(address(connected).balance, initialReceiverBalance + AMOUNT);
        } else {
            assertEq(IERC20(tokenInfo.asset).balanceOf(address(connected)), initialReceiverBalance + AMOUNT);
        }
    }
    ...
    function test_depositAndCall_erc20_fail_and_revert_on_eth() public {
        ...
        emit Connected.RevertEvent("Revert on EVM", RevertContext({
    ...
    function test_withdrawAndCall_zrc20_fail_and_abort_on_zeta() public {
        RevertOptions memory revertOptions = _getDefaultRevertOptions(address(universal), false, address(universal));
        CallOptions memory callOptions = _getDefaultCallOptions(false);
        address zrc20 = eth_eth.zrc20;
        ...
        vm.expectEmit(false, false, false, true);
        emit Universal.AbortEvent("Abort on ZetaChain", AbortContext({
    ...
    function test_withdrawAndCall_zrc20_fail_and_revert_on_zeta() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit Universal.RevertEvent("Revert on ZetaChain", RevertContext({
    ...
    function _getDefaultRevertOptions(address revertAddr, bool callOnRevert, address onAbort) internal pure returns (RevertOptions memory) {
    ...
    function _getDefaultCallOptions(bool isArbitrary) internal pure returns (CallOptions memory) {
        return CallOptions({
            isArbitraryCall: isArbitrary,
            gasLimit: 500000
        });
    }
}
