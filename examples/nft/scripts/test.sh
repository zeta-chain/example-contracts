...
CONNECTED=$(npx hardhat deploy --name Connected --json --network localhost --gateway 0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0 | jq -r '.contractAddress')
...
SENDER=$(npx hardhat transfer --network localhost --contract "$UNIVERSAL" --token-id "$NFT_ID" --json --zrc20 0x2ca7d64A7EFE2D62A725E2B35Cf7230D6677FfEe --receiver "$CONNECTED" | jq -r '.sender')
