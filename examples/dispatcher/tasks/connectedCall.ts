...
const main = async (args: any, hre: HardhatRuntimeEnvironment) => {
  ...
  const functionSelector = args.function
    ? ethers.utils.hexlify(ethers.utils.arrayify(args.function))
    : "0x";

  const message = functionSelector + encodedParameters.slice(2); // Prepend function selector to the message
...
task(
  ...
  .addOptionalParam(
    "function",
    "Bytes function selector to prepend to the message",
    "0x"
  );
